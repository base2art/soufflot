namespace Base2art.Soufflot.Api.Routing.Functional
{
    using System;

    using Base2art.Soufflot.Http;

    public class FunctionalNonRenderingControllerSearchDelegate : INonRenderingControllerSearchDelegate
    {
        private readonly Func<IHttpRequest, Type> mapperFunc;

        public FunctionalNonRenderingControllerSearchDelegate(Func<IHttpRequest, Type> mapperFunc)
        {
            this.mapperFunc = mapperFunc;
        }

        public Type FindType(IHttpRequest request)
        {
            var func = this.mapperFunc;
            if (func != null)
            {
                return func(request);
            }

            return null;
        }
    }
}
